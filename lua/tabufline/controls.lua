...
M.set_filenames = function(tabnr)
	...
	local buf_names = vim.t[tabnr].buf_names or {}
	...
	for _, bufs in pairs(fname_to_buf) do
		...
		if vim.tbl_count(bufs) > 1 then
			...
			local reduced_buf_dir =
				require("tabufline.path").reduce_paths(buf_to_dir)
	...
	vim.t[tabnr].buf_names = buf_names
end
